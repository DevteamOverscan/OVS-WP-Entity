...
if(!class_exists('Field_number')) {
    class Field_number extends Field
    {
        public function render()
        {
            ...
            if(!empty($this->getId())) {
                echo '<div class="form-row row-text">';
                if(!empty($this->getLabel())) {
                    echo '<label for="' . esc_attr($this->getId()) . '">' . esc_attr($this->getLabel()) . '</label>';
                }
                echo '<input type="number" id="' . esc_attr($this->getId()) . '" name="' . esc_attr($name) . '" ' . esc_attr($required) . esc_attr($placeholder) . ' value="' . esc_attr($this->getValue()) . '">';
