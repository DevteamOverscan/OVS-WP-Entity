...
if(!class_exists('Field_hour')) {
    class Field_hour extends Field
    {
        public function render()
        {
            ...
            if(!empty($this->getId())) {
                echo '<div class="form-row row-hour">';
                if(!empty($this->getLabel())) {
                    echo '<label for="' . esc_attr($this->getId()) . '">' . esc_attr($this->getLabel()) . '</label>';
                }
                echo '<input type="time" id="' . esc_attr($this->getId()) . '" name="' . esc_attr($name) . '" ' . esc_attr($required) . esc_attr($placeholder) . ' value="' . esc_attr($this->getValue()) . '">';
